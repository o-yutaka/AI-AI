...
def test_competition_adapter_normalizes_without_sdk_dependency():
    spec = KaggleAgentSecurityAdapter().normalize(
        {"evaluator_version": "v1", "tool_schema": {"x": {}}}
    )
...
def test_generic_compiler_is_deterministic():
    ...
    first = compiler.compile(model_id="m", prompt="p", tool_schema={"t": {}})
    second = compiler.compile(model_id="m", prompt="p", tool_schema={"t": {}})
...
def test_manifest_fingerprint_changes_with_runtime_identity():
    ...
    first = ExperimentManifest(runtime_id="r1", **base).fingerprint()
    second = ExperimentManifest(runtime_id="r2", **base).fingerprint()
    assert first != second
...
def test_compute_selector_respects_vram_and_quota():
    selected = select_compute_target(
        ...
        [
            ComputeTarget("small", True, 8, 100, 0),
            ComputeTarget("fit", True, 12, 60, 1),
        ],
    )
